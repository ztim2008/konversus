...
Каждый день конвейер собирает **до 40 качественных лидов** (сайты компаний), готовит КП lead-web.pro и **автоотправляет** каплями в окне 09–21 МСК (бюджет sent≤40; темп 15 или 30 мин в Рулетке); админка — контроль, пропуск, kill-switch; ответы — в Telegram и админку.

> **Поправка 2026-09-14 (владелец):** автоотправка разрешена; лимит дня **40**; режим «контент-цех». Ручной Send/Skip сохранены.  
> **Поправка 2026-09-21:** спокойный drip — сбор ~2 КП / 30 мин (DeepSeek без залпа); дефолт отправки 30 мин (~24–26/день); для ~40 — интервал 15 мин в Рулетке. Cron → localhost:3010.
...
10. **География:** один активный город на утро из пула; без запросов «вся Россия» в SERP.  
    **Гипотеза B (2026-09-26):** ядро = лесной пояс (Петрозаводск, Вологда, Череповец, Кострома, Киров, Новгород, Тверь); СПб/ЛО/Мск — тонкий слой сбыта. Bootstrap «только СПб» выключен.
...
| 2026-09-17 | Окно наблюдения конвейера по календарю (~17.09); разбор метрик отложен. Этапы 0–9 без смены | 🟩 без изменений |
| 2026-09-26 | Гипотеза B: Алексей/russait · дерево-дома · лесной пояс · call-мост · weak_hosting · hotScore≥40 | этапы 0–9 🟩; правило 10 уточнено |
