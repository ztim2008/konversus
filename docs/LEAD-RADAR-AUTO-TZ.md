...
Каждое утро в общем Telegram и в админке лежит **готовая пачка до 20 качественных лидов** — только **сайты компаний/исполнителей** (не статьи), с аудитом, платформой, **найденным email** (главная + внутренние/контакты), **скриншотом сайта** в КП и в Telegram, и ссылкой на **lead-web.pro**; вы с напарником жмёте «Отправить» / «Пропустить»; ответы — в чат и в админку.

---

## Нецель (жёсткие границы MVP этого ТЗ)

- Автоотправка писем без кнопки человека.
- Более 20 исходящих писем в день (по умолчанию).
...
- [ ] Проблемы в `problems` + `privacy_issues`
- [ ] Платформа видна в админке
- [ ] Email с внутренних/контактов находится на фикстуре «почта не на главной»
- [ ] Скриншот сохраняется и открывается по URL
- [ ] Без email / без скрина → соответствующие skip-статусы
- [ ] Статус → 🟩
...
- [ ] Превью HTML в админке **со скрином**
- [ ] Тестовое письмо на свой ящик: картинка сайта видна
- [ ] Ссылка ведёт на lead-web.pro с leadId
- [ ] Токены пишутся в БД
- [ ] Статус → 🟩
...
- [ ] Один успешный nightly-run на стейдже/проде
- [ ] Утром ≤20 в `queued`, у всех есть screenshot_url и email
- [ ] Строка batches за сегодня создана с `queued_count`
- [ ] Дайджест уходит в чат (текст; фото — этап 7)
- [ ] Статус → 🟩
...
- [ ] Можно закрыть пачку дня без CLI
- [ ] Скрин виден до отправки
- [ ] Skip не отправляет письмо
- [ ] Статус → 🟩
...
- [ ] Оба видят дайджест и отправки
- [ ] При отправке в чат уходит скрин сайта клиента
- [ ] `daily-report` пишет batches и шлёт вечернюю сводку
- [ ] При 0 отправок отчёт не уходит
- [ ] Нет спама на каждое открытие письма
- [ ] Статус → 🟩
...
- [ ] Хотя бы один авто-канал ответов работает end-to-end
- [ ] Статус в БД и сообщение в чат совпадают
- [ ] Статус → 🟩
...
- [ ] Видна сумма токенов за день и история batches
- [ ] Лимит 20 соблюдается кодом, не «на честном слове»
- [ ] Админка и вечерний TG показывают одни и те же метрики дня
- [ ] Статус → 🟩 → MVP Auto закрыт
...
| Критерий | Проверка |
|----------|----------|
| Утром есть очередь ≤20 | БД `queued` + Telegram (план) |
...
| Отправка только вручную | Нет cron send |
