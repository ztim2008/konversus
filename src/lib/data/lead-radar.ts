...
export async function listAllSites(): Promise<any[]> { const db = getDbPool(); const [rows] = await db.query("SELECT * FROM lead_radar_sites WHERE status != "new" ORDER BY contacted_at DESC LIMIT 100"); return rows as any[]; }
